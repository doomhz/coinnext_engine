(function() {
  ...
  OrderBook = {
      ...
      return BuyOrder.find(id, {
        ...
        return OrderBook.findMatchingSellOrders(buyOrderToMatch, transaction, function(err, matchingSellOrders) {
          ...
          return async.mapSeries(matchingSellOrders.concat(buyOrderToMatch), updateOrderCallback, function(err, result) {
            ...
            return GLOBAL.queue.Event.addMatchOrders(matchResults, transaction, function(err) {
              if (err) {
    ...
    calculateResultAmount: function(order, amount, unitPrice) {
      ...
      return math.round(math.multiply(amount, unitPrice));
    },
    calculateFee: function(amount) {
      return math.round(math.select(amount).divide(100).multiply(MarketHelper.getTradeFee()).done(), 0);
    },
    ...
    deleteOpenOrder: function(externalId, callback) {
      ...
      return BuyOrder.destroy(query).complete(function(err, buyRows) {
        if (err || buyRows) {
          return callback(err, buyRows);
        }
        return SellOrder.destroy(query).complete(function(err, sellRows) {
          if (err || sellRows) {
            return callback(err, sellRows);
          }
